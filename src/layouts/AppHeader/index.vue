...
<template>
   <nav class="fixed z-20 top-0 left-0 right-0 blured-background py-3 lg:py-4 shadow-sm">
      <div class="flex items-center container mx-auto px-5 md:px-9 lg:px-16 xl:px-36 2xl:px-52">
      	<!-- Logo -->
         <a href="#intro">
      		<img class="w-[50px] h-[50px] xs:w-[40px] xs:h-[40px]"
             :src="handleLogo" :alt="`${ intro.fullname } logo`">
      	</a>
         <!-- End Logo -->

         <!-- Three bars menu (only apears on mobile) -->
         <button @click="isCollapsed =! isCollapsed" class="ml-auto md:hidden">
            <i v-if="isCollapsed" class="xs:text-[23px] fa fa-bars"></i>
            <i v-else class="xs:text-[22px] fa fa-times"></i>
         </button>
         <!-- End three bars menu -->
         ...
         <!-- Navbar menu -->
      	<ul
            class="
               ...
               md:h-auto w-3/4 md:w-auto p-5 md:p-0 md:flex md:bg-transparent dark:md:bg-transparent md:transform-none items-center ml-auto
            "
         ...
         >
            <!-- Menu links -->
      		<li
      			...
      			v-for="label, anchor, k in links"
      			:key="anchor"
      		>
      			<a
                  class="font-bold text-md md:text-xs lg:text-md" :href="`#${ anchor }`">
      				<span class="text-main-color">0{{ k + 1 }}.</span>
